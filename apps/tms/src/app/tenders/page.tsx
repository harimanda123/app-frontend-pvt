...
import { FileCheck2, Search, Truck, Clock, CheckCircle2, AlertTriangle, ArrowUpRight, Zap } from "lucide-react";
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-ink-muted mt-1 font-medium">
                Automated carrier tender generation, expiration timeouts, and fallback cascades upon rejection.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-border p-6 shadow-2xs">
            {tenders.length === 0 ? (
              <div className="p-12 text-center text-xs text-ink-muted font-medium bg-surface-muted rounded-xl border border-dashed border-border space-y-2">
                <p className="font-bold text-ink">No carrier tenders dispatched yet.</p>
                <p>Dispatched tenders will track expiration countdowns and execute automated fallback cascades.</p>
              </div>
            ) : (
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-surface-muted/50 transition-colors">
                          ...
                          <td className="py-3.5 px-4 font-mono font-semibold text-ink-muted">
                            {t.shipment?.shipmentNumber ?? t.freightQuoteId?.slice(0, 10) ?? "Quote-100"}
                          </td>
                          ...
                          <td className="py-3.5 px-4 font-semibold text-ink">
                            {t.expiresAt ? new Date(t.expiresAt).toLocaleTimeString() : "4 Hours"}
                          </td>
