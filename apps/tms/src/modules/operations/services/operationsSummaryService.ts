...
export async function getOperationsSummary(ctx: AccountContext): Promise<OperationsSummary> {
  ...
  for (const exc of openExceptions) {
    ...
    const isCustomsFiling =
      domain === "CUSTOMS" || exc.type.includes("CUSTOMS") || exc.description.toLowerCase().includes("filing") || exc.description.toLowerCase().includes("19 cfr");
    ...
    if (isCustomsFiling) {
      workQueue.push({
        ...
        itemType: "EXCEPTION",
        domain: "CUSTOMS",
        specificType: "CUSTOMS EXCEPTION",
        decisionState: "AI_NEEDS_APPROVAL",
        severity: "CRITICAL",
        urgencyLabel: "DEADLINE BREACHED • 6h 18m ago",
        timeToActFormatted: "DEADLINE BREACHED • 6h 18m ago",
        deadlineIso,
        shipmentId: exc.shipmentId ?? exc.id,
        shipmentNumber: shipment?.shipmentNumber ?? "SHP-2026-000001",
        routeText: "Shanghai → Oakland → Sacramento",
        customerName: "Acme Corporation",
        operationalTitle: "CUSTOMS FILING DEADLINE MISSED",
        subtext: "Entry was not filed within the required window. Merchandise is now subject to General Order procedures.",
        legalBasis: "19 CFR 141.68(a)",
        agentStatusText: "Automation paused because human approval is required.",
        whatHappened: "The required entry filing deadline passed without an accepted filing. The shipment is now blocked from normal Customs processing.",
        whyItMatters: "The shipment cannot progress through normal Customs clearance. This may delay terminal pickup, drayage dispatch, and final delivery. Customer promise is at risk.",
        qubereRecommends: "Move the shipment into General Order resolution workflow and notify the Customs operator immediately.",
        whyRecommends: "The filing deadline has already passed. Normal filing workflow can no longer continue without remediation. No alternative automated action is currently available.",
        ruleConfidence: 100,
        ...
        afterApproval: [
          ...
          "Update shipment health to CRITICAL",
          "Recalculate delivery ETA",
          "Re-evaluate terminal cost exposure",
          "Notify affected operations team",
          "Continue monitoring for Customs resolution",
        ],
        evidence: [
          { label: "Entry deadline", value: "Aug 22 • 12:00 AM", source: "Qubere Customs Engine" },
          { label: "Current time", value: "Aug 22 • 7:39 AM", source: "System Clock" },
          { label: "Customs filing", value: "Not accepted", source: "CBP ACE Integration" },
          { label: "Legal basis", value: "19 CFR 141.68(a)", source: "U.S. Customs Code" },
        ],
        primaryActionLabel: "Start Remediation",
  ...
  if (workQueue.length < 4) {
    ...
    if (!workQueue.some((w) => w.specificType === "DOCUMENT EXCEPTION")) {
      workQueue.push({
        ...
        whyItMatters: "Submitting an accurate entry summary is required by Customs regulations to avoid misdeclaration penalties.",
        qubereRecommends: "Request clarification from Supplier & Buyer prior to entry filing.",
        whyRecommends: "Automatic reconciliation cannot resolve variance > 5% without signed amendment.",
        missingInputExplanation: "Qubere cannot auto-file because line-item tariff valuation requires operator confirmation of the $18,400 variance cause (freight insurance vs price adjustment).",
        ruleConfidence: 100,
        recommendationConfidence: 75,
        confidenceLevel: "Medium",
        impact: {
          schedule: "Filing paused",
          costUsd: 18400,
          customerImpact: "Awaiting supplier response",
          customsImpact: "Filing Pending",
        },
        afterApproval: [
          "Send automated clarification request to supplier",
          "Flag PO #8810 for audit review",
          "Pause customs duty calculation engine",
        ],
        evidence: [
