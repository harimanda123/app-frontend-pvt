...
| 7 | Drawback (TFTEA) | Strong existing foundation (claims/matches/lots are real) | **Codec built** (`src/lib/abi/drawback/`) — full 21-input/2-output backbone (Records 10, 31, 40-43, 50-53, 60-64, 70-73, 89-90, output E0/E1). Money fields moved to `Decimal` (Record 31's STB Bond Amount whole-dollar; Record 42/60/70 quantity & per-unit value fields 4-implied-decimal; Record 43/89/90 revenue amounts 2-implied-decimal — all confirmed against the source PDF, including Record 60's Quantity field which the field-derivation pass hadn't explicitly annotated), MMDDYY dates moved to `Date` (Records 40, 50, 60/70, 62, 63, 64), and ITIN/MTIN/ETIN cross-record join keys plus CBP ES Line #, Accounting Class Code, and Drawback Accounting Method Code moved off raw `parseInt` decode onto `numericCodeField` so a leading zero (e.g. `"00042"`) no longer silently collapses to `42` and breaks the Record 40↔52/72 and 50↔53/73 joins. **Data-quality caveat**: independent verification against `07-drawback-tftea-v27.pdf` during this pass found Records 50, 51, 60, 62, 63, 64, and 70 don't match the PDF's actual field layout (extra/missing fields, e.g. no "Export Tracking ID Number" or "Export Value Per Unit" field exists in the real Record 60/70 at all; Record 50 is missing Action Indicator/Quantity/UOM/Factory Location and has a phantom MTIN; Records 61/71 have their two fields transposed) — the current codec matches the two already-audited test files exactly (by design, per this pass's brief) but does **not** reflect the real wire format for those 7 records; Records 10, 31, 40, 41, 42, 43, 52, 53, 72, 73, 89, 90, E0, E1 were independently spot-checked and do match the PDF. A follow-up re-derivation pass against the PDF is needed for the flagged records before this chapter is trusted for real CBP transmission. Transport not wired. |
